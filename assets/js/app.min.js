var App = (function () {
  ...
  function parseMetaHttp() {
    // parseMetaHttp – experimental http code path & base 
    // jQuery script
    // return Object.fromEntries(
    //         $('meta[name=x-http]').attr('content').split(', ').map(x => x.split('=').map(s => s.trim()))
    //     )
  ...
  function init() {
    ...
    if (path && !initializedModules[path]) {
      var modulePath = parseHttp.base + 'modules/' + path + '.js?_=' + timestamp;
  ...
  return {
    ...
    init:init,
    parseHttp:parseMetaHttp,
    getLogin:getModalLogin,
    getLogout:getModalLogout,
    pages: {}
  };
})();
App.dispatch()