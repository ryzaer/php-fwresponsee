...
(function ($) {
    $.fn.simpleAutocomplete = function (options) {
        ...
        function addNotExistLabel() {
            var latest = settings.data.reduce(function (prev, current) {
                return (prev.id > current.id) ? prev : current;
            }, { id: 0 });
            // panggil callback onNotExistLabel
            // nilai true/false dikirim ke callback
        ...
        $input.on("keydown", function (e) {
            ...
            if (key === 38) { // Up
                ...
                if ($selectedItem.length) {
                    e.preventDefault();
                    // jika ada pilihan sugesti munculkan icon clear
                    $clearIcon.show();
                    ...
                    if ($input.val())
                        if(typeof settings.onNotExistLabel !== 'function'){
                            $input.trigger('blur'); // trigger blur untuk validasi
                        }else{
                            // not exist disini
                            addNotExistLabel()
                        }
