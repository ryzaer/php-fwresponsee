/**
 * Plugin ini mencakup:
 * .panelWrapper – panel segments
 * .animateToThetop – smooth scroll ke atas
 * .arrayToTable – konversi array ke tabel HTML
 * .xhtml – sanitasi HTML pakai DOMPurify
 * .xbind – helper off/on event
 * .assign – kepeluan login my respponsee template (pengembangan)
...
(function($) {
    ...
    */
    ...
    // panel wrapper segments
    $.fn.panelWrapper = function(){
        $(this).on('change', function(){
            let selectedId = $(this).attr('id');
            // Dapatkan kontainer terdekat (wrapper)
            let wrapper = $(this).closest('.panel-wrapper');
            // Sembunyikan semua panel dalam wrapper itu
            wrapper.find('.panel-segment').removeClass('active');
            // Tampilkan panel yang sesuai dengan id radio
            wrapper.find('#panel-' + selectedId).addClass('active');
        });
    };
    // tooltips
    $.fn.eventTooltips = function () {
        $(this).hover(function() {
            var a = $(this).position(),
                e = $(this);
            a = e.offset();
    ...
    $.fn.customInputPasswd = function(options){
        const settings = $.extend({
            maskChar: "•" // default masking char
        }, options);

        return this.each(function(){
            const $input = $(this);

            // cek apakah sudah ada data-text
            if (typeof $input.attr('data-text') === "undefined") {
                $input.attr('data-text', ""); // buat atribut kosong
            }

            $input.on('input.customInputPasswd', function(e){
                let realText = $input.data('text') || "";
                ...
                // simpan teks asli ke data dan atribut
                $input.data('text', realText);
                $input.attr('data-text', realText);

                // tampilkan masking char sesuai panjang teks
                masked = settings.maskChar.repeat(realText.length);
                $input.val(masked);
            });
        });
    };
    // untuk kepeluan login my respponsee template
    $.fn.initHistory = function(){
        $(this).historyCetak(function(prop){
            prop.off('click').on('click',function(){
                $.formBiodata('open',$(this).data('id'));
            })
        });
    };
    $.fn.assign = function(options){
        let session = $.sessionHandler(),
            parseHttp = App.parseHttp(),
            loopProp = this,
            loopAssign = function(){
                if(session.get('token')){
                    $.post(parseHttp.base + 'assign/status',{token:session.get('token')}).done((response)=>{
                        // console.log(response)
                        session.set('token',response.result.token);
                        // terapkan nilai ke form operator
                        let fdata = JSON.parse($.base64.decode(response.result.hash));
                        $('input[name=nickname]').attr('data-id',fdata.id).attr('data-nick',fdata.nick).val(fdata.nick);
                        $('input[name=realname]').val(fdata.nama);
                        $('input[name=usr_phone]').val(fdata.hp);
                        $('input[name=usr_email]').val(fdata.email);
                        if(typeof options === 'function'){
                            options.apply(loopProp);
                        }
                    }).fail((xhr)=>{
        ...
        // jika app merupakan spa perlu dikembangkan lagi
        loopAssign();
        var checkStatus = setInterval(loopAssign,60000*5);        
    };
    // ini untuk chain selector area
    $.fn.chainSelectorArea = function($true=true){
        return this.each(function(){
            // parent harus memiliki child dengan properti2 name area  
            // kasi waktu jeda jika ada trigger events di awal
            const $selectKab = $(this).find('select[name=kabupaten]'),
                  $selectKec = $(this).find('select[name=kecamatan]'),
                  $selectKel = $(this).find('select[name=kelurahan]');
            if($true){
                $selectKab.on('change',function(){
                    $.getJSON('wilayah/kec/'+$(this).val(),function(vars){                            
                        ...
                        $selectKec.val('').attr('disabled',$selectKab.val() ? false : true).simpleSelect('reload'),
                        $selectKel.val('').attr('disabled',true).simpleSelect('reload');
                    })
                });
                $selectKec.on('change',function(){
                    $.getJSON('wilayah/kel/'+$(this).val(),function(vars){
                        ...
                        $selectKel.val('').attr('disabled',$selectKec.val() ? false : true).simpleSelect('reload')
                    })
                });
            }else{
                $selectKab.off('change'),
                $selectKec.off('change')
            }            
        })
    }
})(jQuery);
Array.prototype.for_each = function(callback) {
  const arr = this; // refer ke array yang sedang dipanggil
